...
float castRay(vec3 eye, vec3 dir, out int material)
{
	material = M_NONE;
	...
	{
		vec3 pos = dir * totalDist + eye;
		float dist = SD_Scene(pos, m);
		if(dist < epsilon * totalDist || totalDist > maxDist)
			break;
...
float calcShadow(vec3 eye, vec3 dir, float minDist, float maxDist)
{
	int m;
	for (float totalDist = minDist; totalDist < maxDist;)
    {
        float dist = SD_Scene(eye + dir * totalDist, m);
        if(dist < epsilon) return 0.0;
        totalDist += dist;
    }
    return 1.0;
}
float calcSoftshadowCheap(vec3 eye, vec3 dir, float minDist, float maxDist, float k)
{
	int m;
    float res = 1.0;
	for (float totalDist = minDist; totalDist < maxDist;)
...
float calcSoftshadow(vec3 eye, vec3 dir, float minDist, float maxDist, float k)
{
	int m;
    float res = 1.0;
	float ph = 1e20;
...
vec3 render(vec3 eye, vec3 dir)
{
	...
	int material;
	float dist = castRay(eye, dir, material);

	if(material != M_NONE)
	{
		vec3 pos = dir * dist + eye;
...
void main()
{
	...
	vec3 dir = normalize(
		cross(camRight, camUp) // camForward
 		* focalLength + camRight * fragPos.x * 
		(resolution.x / resolution.y) + camUp * fragPos.y);
